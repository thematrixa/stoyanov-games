...
export class AddEditProductComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.product.name) {
      //gotta check if a some field is full so i know that its not a a new product
      ...
      this.addProductForm.patchValue({
        tournamentStoreLaunchDate: this.dateFormatter.transform(
          this.product.tournamentStoreLaunchDate
        )
      });
      this.addProductForm.patchValue({
        launchDate: this.dateFormatter.transform(this.product.launchDate)
      });
      this.addProductForm.patchValue({
        konamiTournamentLegalDate: this.dateFormatter.transform(
          this.product.konamiTournamentLegalDate
        )
      });
      this.addProductForm.patchValue({
        cardsPerPack: this.product.cardsPerPack
      });
  ...
  onSubmit() {
    ...
    let product = this.productService.generateProductFromForm(
      this.addProductForm
    );
